...
std::default_random_engine eightQueenState::engin(static_cast<unsigned long>(time(0)));
const double eightQueenState::epsilon = 0.1;

eightQueenState eightQueenState::operator+(const eightQueenState &right) const
{
...
bool eightQueenState::operator <(const eightQueenState &right) const
{
...
std::set<eightQueenState> eightQueenState::successors() const
{
    std::set<eightQueenState> res;
    std::uniform_int_distribution<int> u(0,N-1);
    int i = u(engin);
    auto tmp = plane;
    for(int j=0;j<N;j++)
    {
        tmp[i]=j;
        if (tmp != plane)
            res.insert(eightQueenState(tmp));
    }
    return res;
}

std::set<eightQueenState> eightQueenState::successors(int v) const
{
    std::set<eightQueenState> res;
    auto tmp = plane;
    for(int j=0;j<N;j++)
